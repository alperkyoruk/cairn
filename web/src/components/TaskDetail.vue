...
import TransitionActions from './TransitionActions.vue'
import ConfirmDialog from './ConfirmDialog.vue'
import RelativeTime from './RelativeTime.vue'
import { formatAbsolute } from '../composables/useRelativeTime.js'

// One component, two hosts: the full page at /t/{ref} and the drawer over the
// board. Everything below -- the state panel, the worklog, the transition
// forms, edit and delete -- is shared, because a second thinner task view is a
// thing that drifts, and the half that drifts is always the one with the rules
// in it.
const props = defineProps({
  taskRef: String,
  // In a drawer the rail cannot sit beside the content at 600px, and deleting
  // cannot navigate the page underneath out from under itself.
  embedded: { type: Boolean, default: false },
})
const emit = defineEmits(['moved', 'deleted'])
...
async function move(to) {
  ...
  try {
    ...
    pending.value = null
    // The board behind the drawer is now stale in one row. It decides what to
    // do about that -- reload and close, usually -- because it owns the
    // selection and the filter and this component knows about neither.
    emit('moved', detail.value.task)
  } catch (err) {
    failure.value = err.message
  } finally {
    busy.value = false
...
<template>
  ...
  <div v-else class="task" :class="{ embedded }">
    <div class="content">
...
/* In a drawer there is no room for a rail beside the content, and no reason
   for one: the drawer is already the narrow column the rail was carving out.
   The actions go first instead of to the side, because in a drawer you are
   there to decide rather than to read. */
.task.embedded {
  display: flex;
...
.task.embedded .rail { border-left: 0; border-bottom: 1px solid var(--rule); width: auto; }
.task.embedded .pad { padding: var(--s-6); }
.pad { padding: var(--s-8); }
